...
const Cell = ({ icon: Icon, label, value }: { icon: any; label: string; value: string }) => (
...
export const CaseSummary = ({ data }: { data: SourcingCase }) => (
  <Card className="overflow-hidden border-border/70 shadow-elegant bg-card animate-fade-up">
    <div className="flex divide-x divide-border/60">
      ...
      <Cell icon={Package} label="Product" value={data.product} />
      <Cell icon={FlaskConical} label="Ingredient" value={data.ingredient} />
      <Cell icon={Truck} label="Supplier" value={data.supplier} />
    </div>
    <div className="flex items-center gap-3 px-5 py-3 bg-secondary/40 border-t border-border/60">
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="text-xs px-2 py-0.5 rounded-full bg-card border border-border/70 text-foreground">Price · {data.preferences.price}</span>
        <span className="text-xs px-2 py-0.5 rounded-full bg-card border border-border/70 text-foreground">Lead · {data.preferences.leadTime}</span>
        <span className="text-xs px-2 py-0.5 rounded-full bg-card border border-border/70 text-foreground">Quality · {data.preferences.quality}</span>
